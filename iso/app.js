...
/*
  ISO SHAPE PROTOTYPE
  =====================
  Three variables, each independently selectable:
    1. Light direction (uLightDir) - shared across all shapes
    2. Colour gradient (multiply-blend overlay) - shared across all shapes
    3. Shape - each shape defines its own vertex shader, geometry,
       uniform mapping (from `value`), and corner/face data used for
       the live-preview overlay sizing and SVG export.

  Fragment shader (greyscale lambert) and the gradient-overlay /
  SVG-export plumbing are SHARED - only the SHAPES registry below
  needs a new entry to add a shape-transformation.

  ---- ADDING A NEW SHAPE ----
  Add an entry to SHAPES with:
    - label: dropdown text
    - geometry(): returns a THREE.BufferGeometry (unit-space)
    - vertexShader: GLSL, must set `vNormal` and `gl_Position`
        using projectionMatrix * modelViewMatrix
    - getUniforms(value): shape-specific uniforms (besides uLightDir)
    - getPoints(value): named 3D corner points (object-space, same
        space the vertex shader scales into) used for overlay bbox
        + SVG projection
    - getFaces(value): [{ labels: [...point names, in order], normal: [x,y,z] }]
      for the *visible* faces only, used for SVG polygons. Normals
      need not be pre-normalised. Takes `value` since some shapes'
      face normals change as the shape morphs.
*/

// ---- Shared shading constants ----
const AMBIENT = 0.45;
const DIFFUSE = 0.55;
const ISO_DEG = 30;
const LIGHT_PRESETS = {
...
// DISPLAY_SCALE enlarges the on-screen preview only; internal render
// resolution stays 68x54.
const CANVAS_W = 68;
const CANVAS_H = 54;
// Render at higher pixel density while keeping the same world-space
// framing (FRUSTUM_D, aspect) - i.e. "what 68x54 looks like on a
// PIXEL_SCALE-x screen". Displayed 1:1 at native resolution, so no
// CSS upscaling blur.
const PIXEL_SCALE = 4;
// Orthographic camera frustum half-height (world units). Controls how
...
const SHAPES = {
  adsSustain: {
    ...
    vertexShader: `
      ...
      void main() {
        // unit BoxGeometry: position in [-0.5, 0.5] per axis.
        // x,z scaled & centred; y rescaled so y=0 (bottom) stays fixed.
        vec3 scaled = vec3(
          position.x * uLen,
          (position.y + 0.5) * uHeight,
          position.z * uDepth
        );
  ...
  adsDecay: {
    ...
    vertexShader: `
      ...
      void main() {
        vec3 p = vec3(position.x * uLen, position.y * uHeight, position.z * uDepth);
        // correct the normal for the non-uniform (uLen,uHeight,uDepth) scale
        vec3 n = normalize(normal / vec3(uLen, uHeight, uDepth));
...
// ---- Alignment ----
// ADS 03 Sustain's bbox (centre-x and bottom-y) is constant across its
// value range (fixed footprint), so it's used as the reference position.
// Other shapes are shifted (in NDC, post-projection) so their bbox
...
export default function IsoShapePrototype() {
  ...
  const rebuildMesh = (currentShape, currentValue, currentLightKey) => {
    ...
    const material = new THREE.ShaderMaterial({
      vertexShader: currentShape.vertexShader,
  ...
  // shape change -> rebuild mesh entirely
  useEffect(() => {
    if (!sceneRef.current) return;
    rebuildMesh(shape, value, lightKey);
    updateOverlay(shape, value);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shapeKey]);

  // value change -> update uniforms + overlay
  useEffect(() => {
    if (materialRef.current) {
      ...
      const cam = cameraRef.current;
      if (cam) {
        const offset = getNdcOffset(shape, value, cam);
        materialRef.current.uniforms.uNdcOffset.value.set(offset.x, offset.y);
      }
    }
    updateOverlay(shape, value);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  ${overlayRectSvg}
    ...
    a.href = url;
    a.download = `${shapeKey}_${value}_${lightKey}_${gradientIndex}.svg`;
    a.click();
